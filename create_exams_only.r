...
create_tests_and_collect_metadata <- function() {
  ...
  for (i in 1:num_tests) {
    ...
    # Generate a unique external test ID using UUID
    external_test_id <- sprintf("test_%s", substr(UUIDgenerate(), 1, 8))
    title <- sprintf("%s %s", title_prefix, format(test_start_time, "%Y-%m-%d %H:%M"))
